...
-- Task 22.3 follow-up: make day_count a required field for
-- 'increasing_daily_capped' plans. The UI now treats
-- "Maximum daily amount" and "Run this plan for" as core
-- fields on the Increasing Daily preset, so the server must
-- reject capped revisions that omit day_count.
--
-- The bare 'increasing_daily' rule remains unchanged for
-- backwards compatibility with any pre-existing revisions.
-- ============================================================

begin;

-- ── extend rule-field check to require day_count on capped ─────
  ...
  check (
    case rule_type
      ...
      when 'increasing_daily_capped' then start_amount is not null and start_amount > 0
                                          ...
                                          and day_count is not null and day_count > 0
      else false
    end
  );

-- ── recreate create_saving_plan with day_count required for capped ──
...
begin
  ...
  if p_rule_type in ('fixed_daily','fixed_weekly','fixed_monthly') then
    ...
    if p_rule_type = 'increasing_daily_capped' then
      ...
      if p_day_count is null or p_day_count <= 0 then
        raise exception 'day_count is required and must be positive for increasing_daily_capped' using errcode = '22023';
      end if;
    end if;
  end if;
...
-- ── recreate change_saving_plan with day_count required for capped ──
