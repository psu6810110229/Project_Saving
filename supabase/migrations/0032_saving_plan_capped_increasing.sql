...
-- Task 22.3 Advanced Saving Plan Formulas.
--
-- Adds 'increasing_daily_capped' rule_type support to
-- saving_plan_revisions, plus cap_amount handling and
-- additional validation in create_saving_plan and
-- change_saving_plan:
--   - cap_amount must be > 0 when provided
--   - capped rule requires start_amount, increment_amount, cap_amount
--   - cap_amount must be >= start_amount for capped rule
--   - end_date must be on or after effective_from_date (kept from 0031)
--   - day_count must be > 0 when provided (table-level check)
--
-- Both RPCs are dropped and recreated to extend the signature
-- with p_cap_amount. All other behavior (membership check,
-- auth.uid guard, search_path lockdown, append-only revisions)
-- is preserved.
-- ============================================================

begin;

-- ── extend rule_type whitelist on the table ────────────────────
alter table public.saving_plan_revisions
  drop constraint if exists saving_plan_revisions_rule_type_check;

alter table public.saving_plan_revisions
  add constraint saving_plan_revisions_rule_type_check
  check (rule_type in (
    'fixed_daily',
    'fixed_weekly',
    'fixed_monthly',
...
-- ── extend rule-field check to cover capped rule ───────────────
alter table public.saving_plan_revisions
...
-- ── drop old RPC signatures (replaced below with cap support) ──
drop function if exists public.create_saving_plan(uuid, text, numeric, numeric, numeric, numeric, date, date, int);
drop function if exists public.change_saving_plan(uuid, text, numeric, numeric, numeric, numeric, date, date, int);

-- ── create_saving_plan RPC (Task 22.3 with cap_amount) ─────────
...
-- ── change_saving_plan RPC (Task 22.3 with cap_amount) ─────────
