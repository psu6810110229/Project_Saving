 ...
 *
 * The list does NOT introduce horizontal scrolling, virtualisation,
 * or row navigation. Rows remain non-interactive except for the
 * optional trailing slot (used by the Dashboard to render a per-row
 * `NudgeButton`).
 */
        ...
        {sorted.map((entry, index) => {
          ...
          return (
            <PlayerProgressRow
              ...
              isLeader={isLeader}
              gapLabel={gapLabel}
              trailing={!entry.isYou ? renderRowTrailing?.(entry) : undefined}
            />
          );
        })}
      </div>
